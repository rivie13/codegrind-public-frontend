...
export default function useTowerDefenseTerminalGameAndCode({
  ...
  const handleGameCommand = useCallback(
    (action = 'help') => {
      ...
      if (normalized === 'start-wave') {
        ...
        const canStart =
          ((gameState.status === GAME_STATUS.READY && initialCodeGenerated) ||
        ...
        if (!canStart) {
          addTerminalSystemMessage(
            'WARNING',
            'Wave start unavailable. Deploy core towers or complete current wave first.'
          );
          return true;
        }
      ...
      if (normalized === 'status') {
        ...
        addTerminalSystemMessage(
          'SYSTEM',
          `Status: ${gameState.status} | Wave: ${gameState.wave}/${totalWaves}`
        );
        addTerminalSystemMessage(
          'SYSTEM',
          `Bits: ${gameState.credits} | Lives: ${gameState.lives} | Selected: ${selectedLabel}`
        );
        return true;
      }
  ...
  const handleCodeCommand = useCallback(
    (action = 'help') => {
      const normalized = (action || 'help').toLowerCase();
      if (normalized === 'help') {
        handleCodeHelp();
      ...
      if (normalized === 'test') {
        ...
        if (!shouldShowVerificationControls) {
          addTerminalSystemMessage(
            'WARNING',
            'Code execution unlocks right before the final wave. Complete the current wave to proceed.'
          );
          return true;
        }
        handleRunCode?.();
      ...
      if (['output', 'stdout', 'stderr'].includes(normalized)) {
        if ((normalized === 'stdout' || normalized === 'stderr') && !allowStdStreams) {
          addTerminalSystemMessage(
            'WARNING',
            'Stdout/stderr capture is only available in learning path mode. Use /code output instead.'
          );
          return true;
        }
        if (gameState.status === GAME_STATUS.PLAYING) {
        ...
        if (!shouldShowVerificationControls) {
          addTerminalSystemMessage(
            'WARNING',
            'Code output unlocks right before the final wave. Complete the current wave to proceed.'
          );
          return true;
        }
      ...
      if (normalized === 'submit') {
        if (!shouldShowVerificationControls) {
          addTerminalSystemMessage(
            'WARNING',
            'Verification is only available right before the final wave starts.'
          );
          return true;
        }
        handleSubmitSolution?.();
